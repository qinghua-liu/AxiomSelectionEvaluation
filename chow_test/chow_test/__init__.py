...
def f_value(y1, x1, y2, x2):
    ...
    def find_rss(y, x):
        ...
        coeff, rss, _, _ = np.linalg.lstsq(A, y, rcond=None)
        length = len(y)
        return (coeff, rss, length)

    coeff_total, rss_total, n_total = find_rss(
        np.append(y1, y2), np.append(x1, x2))
    coeff_1, rss_1, n_1 = find_rss(y1, x1)
    coeff_2, rss_2, n_2 = find_rss(y2, x2)

    chow_nom = (rss_total - (rss_1 + rss_2)) / 2
    chow_denom = (rss_1 + rss_2) / (n_1 + n_2 - 4)
    return chow_nom / chow_denom, coeff_total, coeff_1, coeff_2
